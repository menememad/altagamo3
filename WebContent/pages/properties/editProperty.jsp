...
<div id="container" dir='<%=dir%>'> 
	<h2><s:text name="title.property.edit" /></h2>
	<s:form id="editPropertyForm" action="editProperty" method="POST" enctype="multipart/form-data" theme="simple">
	...
	<s:if test="hasFieldErrors()">
	 	<h3 style="color: red"><img src='<s:url includeContext="false" value="/res/tgm3/images//signs/error.gif" />' /><s:text name="err.message" /></h3>
	   	<s:fielderror />
	</s:if>
	<s:elseif test="hasActionErrors()">
		 <h3><img src='<s:url includeContext="false" value="/res/tgm3/images//signs/error.gif" />' /><s:text name="err.message" /></h3>
	   	<s:actionerror />
	</s:elseif>
	<s:elseif test="hasActionMessages()">
	   <img src='<s:url includeContext="false" value="/res/tgm3/images//signs/success.gif" />' /><h3><s:actionmessage /></h3>
	</s:elseif>
	<fieldset>
		...
		<table class="propertyForm">
			...
			<tr class="odd">
				...
				<td width="30%"><sj:radio id="propertyFor" name="property.propertyFor" list="#{1:getText('propertyfor.'+1),2:getText('propertyfor.'+2),3:getText('propertyfor.'+3)}" onSuccessTopics="showRentSaleItems" onChangeTopics="showRentSaleItems"/></td>
			...
			<tr class="even subtype" id="subtype1">
				...
				<td colspan="4"><s:radio name="aprtSubtype" list="#{1:getText('aprt.subtype.'+1),2:getText('aprt.subtype.'+2),3:getText('aprt.subtype.'+3),4:getText('aprt.subtype.'+4),5:getText('aprt.subtype.'+5),6:getText('aprt.subtype.'+6)}"  /></td>
			...
			<tr class="even">
				...
				<td width="30%" class="furnitureStatus"><s:radio name="property.furnitureStatus" list="#{1:getText('aprt.furn.sts.'+1),2:getText('aprt.furn.sts.'+2),3:getText('aprt.furn.sts.'+3)}" /></td>
				...
				<td width="30%" class="finishingStatus"><s:radio name="property.finishingStatus" list="#{1:getText('finish.sts.'+1),2:getText('finish.sts.'+2),3:getText('finish.sts.'+3)}" /></td>
	...
	<fieldset>
		...
		<table class="propertyForm">
			<tr class="odd">
				...
				<td width="30%"><sj:select href="listCountries" id="country" name="property.country.id" list="countriesList" listKey="id" listValue="fullName" emptyOption="false" headerValue="-- Select Country --" headerKey="-1" onSuccessTopics="reloadStatesList" onChangeTopics="reloadStatesList" /></td>
				...
				<td width="30%"><sj:select href="listStates" id="state" name="property.state.id" list="statesList" listKey="id" listValue="fullName" emptyOption="false" headerValue="-- Select State --" headerKey="-1" reloadTopics="reloadStatesList" onSuccessTopics="reloadCitiesList" onChangeTopics="reloadCitiesList" /></td>
	...
	<fieldset>
		...
		<table class="propertyForm">
			...
			<tr class="even">
				...
				<td class="paymentType" width="30%"><sj:radio id="paymentType" name="property.paymentType" list="#{1:getText('payment.type.'+1),2:getText('payment.type.'+2)}" onSuccessTopics="reloadPaymentOptions" onChangeTopics="reloadPaymentOptions"/></td>
				...
				<td class="rentalPeriod" width="30%"><s:radio name="property.rentalPeriod" list="#{1:getText('rental.period.'+1),2:getText('rental.period.'+2),3:getText('rental.period.'+3),4:getText('rental.period.'+4),5:getText('rental.period.'+5),6:getText('rental.period.'+6)}" /></td>
...
$.subscribe('propertyTypeChanged', function(event,data) {
	...
	if($("#propertyType option:selected").val()=='1'){	
		$('#subtype1').show();
		$('#subtypeLabel1').show();
		//$('input:radio[name="aprtSubtype"][value="3"]').attr('checked',true);
		$('.floorNo').show();
		$('.rooms').show();
	...
	if($("#propertyType option:selected").val()=='2'){	
		$('#subtype2').show();
		$('#subtypeLabel2').show();
		//$('input:radio[name="villaSubtype"][value="3"]').attr('checked',true);
		$('.rooms').show();
		$('.furnitureStatus').show();
		$('.finishingStatus').show();
	}
